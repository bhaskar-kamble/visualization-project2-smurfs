...
ggplot(MFH_SFH_spez_verbrauch,aes(y=spz_verbrauch,x=abrechnungsjahr,color=gtype))+geom_point()+
        ...
        stat_function(fun=equation2,geom="line",color=scales::hue_pal()(2)[2])+labs(title="MFH & SFH")+scale_x_continuous(name="Year", breaks =seq(2002, 2018, 2))+scale_y_continuous(name="Specific energy consumption (kWh/m2)",limits=c(0,150)) 
...
When we use interaction between the year and the building type we get a better fit to the data.
...
# for 3rd
equation3=function(x){coef(fit3)[2]*x+coef(fit3)[1]+coef(fit3)[3]+coef(fit3)[4]}

# for 4th
equation4=function(x){coef(fit3)[2]*x+coef(fit3)[1]+coef(fit3)[3]+coef(fit3)[4]+coef(fit3)[5]}
...
ggplot(MFH_spez_verbrauch,aes(y=spz_verbrauch,x=temp,color=as.factor(yearbin)))+geom_point()+
        ...
        labs(title="EFFECT OF TEMPERTURE IN MFH")+scale_x_continuous(name="temp", limits=c(-2,5))+scale_y_continuous(name="Specific energy consumption (kWh/m2)",limits=c(0,150))
```
...
# for 3rd
equation3=function(x){coef(fit4)[2]*x+coef(fit4)[1]+coef(fit4)[3]+coef(fit4)[4]}

# for 4th
equation4=function(x){coef(fit4)[2]*x+coef(fit4)[1]+coef(fit4)[3]+coef(fit3)[4]+coef(fit4)[5]}
...
The following figure shows the proportions of energy produced by the different fuels.

```{r,fig.cap="\\label{fig:figs}Fig. 5"}
...
The following code shows the co2 emitted in Germany.

```{r,fig.cap="\\label{fig:figs}Fig. 6"}
...
```{r,fig.cap="\\label{fig:figs}Fig. 7: Interactive plot for the specific energy consumption for the 16 federal states for multifamily houses from 2002-2018."}
p
```
...
```{r,fig.cap="\\label{fig:figs}Fig. 8: Heat map of the specific energy consumption for the year and the state for multifamily houses."}
library(plotly)
...
```{r,fig.cap="\\label{fig:figs}Fig. 9: Slopes of the linear trends in specific energy consumption for the 16 federal states for multifamily houses in Germany"}
...
```{r,fig.cap="\\label{fig:figs}Fig. 10: Slopes of the linear trends in specific energy consumption for the 16 federal states for multifamily houses in Germany shown as an interactive map. Please click on the respective state to see the state name and the slope value in kWh/Year."}
...
```{r,fig.cap="\\label{fig:figs}Fig. 11:  Interactive plot for the specific energy consumption for the 16 federal states for 1-2 family houses from 2002-2018."}
...
```{r,fig.cap="\\label{fig:figs}Fig. 12:  Heat map of the specific energy consumption for the year and the state for 1-2 family houses."}
...
```{r,fig.cap="\\label{fig:figs}Fig. 13:   Slopes of the linear trends in specific energy consumption for the 16 federal states for 1-2 family houses in Germany"}
...
# 7a. Percentual consumption and CO2 Emissions by Bundesland (MFH & SFH)
...
# 7b. Percentual consumption and CO2 Emissions by Bundesland (MFH)
...
# 7c. Percentual consumption and CO2 Emissions by Bundesland (SFH)
```{r}
