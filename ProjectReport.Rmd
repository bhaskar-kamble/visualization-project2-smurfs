...
absolute_freq <- function(data) {
  T <- aggregate(data, by=list(data$abrechnungsjahr, data$energietraeger), FUN=length)
  data_aggregated = T[c('Group.1', 'Group.2', 'sto_plz')]
...
i1 <- ggplot(data=absolute_freq(DL_MFH), aes(x=abrechnungsjahr , y=value, fill=energietraeger)) +
  geom_bar(stat="identity") + ggtitle('MFH') +  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),legend.position="none")

i2 <-  ggplot(data=absolute_freq(DL_SFH), aes(x=abrechnungsjahr , y=value, fill=energietraeger)) +
  geom_bar(stat="identity") + ggtitle('SFH') +  theme_bw() +
...
relative_freq <- function(data) {
  T <- aggregate(data, by=list(data$abrechnungsjahr, data$energietraeger), FUN=length)
  by_year <- aggregate(data, by=list(data$abrechnungsjahr), FUN=length)
  by_year <- by_year[]
  data_aggregated = T[c('Group.1', 'Group.2', 'sto_plz')]
  colnames(data_aggregated) <- c('abrechnungsjahr', 'energietraeger', 'value')
  for (year in c(2002:2018)) {
    data_aggregated[data_aggregated$abrechnungsjahr == year, ]$value <- 
      data_aggregated[data_aggregated$abrechnungsjahr == year, ]$value / by_year[by_year$Group.1 == year, ]$sto_plz
  }
  return(data_aggregated)
}

i1 <- ggplot(data=relative_freq(DL_MFH), aes(x=abrechnungsjahr , y=value, fill=energietraeger)) +
  geom_bar(stat="identity") + ggtitle('MFH') +  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),legend.position="none")
...
absolute_freq_interactive <- function(data) {
  T <- aggregate(data, by=list(data$abrechnungsjahr, data$energietraeger, data$bundesland), FUN=length)
  data_aggregated = T[c('Group.1', 'Group.2', 'Group.3' ,'sto_plz')]
...
bundlands_energietraeger <- absolute_freq_interactive(DL_MFH)
bundlands_energietraeger


bundlands_energietraeger[bundlands_energietraeger$bundesland == states[3],]$fluessiggas
bundlands_energietraeger[is.na(bundlands_energietraeger)] <- 0


p <- plot_ly(bundlands_energietraeger , type = 'bar') 
v = T
ss = 1
  for (state in states) {
    assign(paste0('data',ss), bundlands_energietraeger[bundlands_energietraeger$bundesland == state,])
    ss = ss + 1
  }
...
p <- plot_ly(type = 'bar') 
  ...
  for (state in states) {
    print(bundlands_energietraeger[bundlands_energietraeger$bundesland == state,]$erdgas)
    a <- which(bundlands_energietraeger$bundesland == state)
       p <- p %>% add_trace(y = ~erdgas[a], name = 'Erdgas',  
                            x = 2002:2018, visible = v)
      p <- p %>% add_trace(y = ~fluessiggas[a], name = 'Fluessiggas',  
                           x = 2002:2018, visible = v)
       p <- p %>% add_trace(y = ~heizoel[a], name = 'Heizoel',  
                            x = 2002:2018, visible = v)
      p <- p %>% add_trace(y = ~holzpellets[a], name = 'Holzpellets',  
                           x = 2002:2018, visible = v)
       p <- p %>% add_trace(y = ~strom[a], name = 'Strom',  
                            x = 2002:2018, 
                            visible = v)
      p <- p %>% add_trace(y = ~waerme[a], name = 'Waerme',  
                           x = 2002:2018, visible = v)

    v = F
  }
#  add_trace(y = ~bundlands_energietraeger[bundlands_energietraeger$bundesland == states[3],]$erdgas, name = "Erdgass", x = 2002:2018,
#           y = c(0,max(bundlands_energietraeger[bundlands_energietraeger$bundesland == states[2],]$fluessiggas))) %>%
 
p <- p %>% layout(
    xaxis = list(range = c(2002, 2018)),
#    yaxis = list(range = c(0, 2500)),
    barmode='stack')

bundlands_energietraeger[bundlands_energietraeger$bundesland == states[2],]
ll <- list()
hh <- list()
i = 2
s = 1
...
relative_freq_interactive <- function(data) {
  ...
  by_year <- aggregate(data, by=list(data$abrechnungsjahr, data$bundesland), FUN=length)
  ...
  for (year in c(2002:2018)) {
    ...
    data_aggregated[data_aggregated$abrechnungsjahr == year & data_aggregated$bundesland == state, ]$value <- 
      data_aggregated[data_aggregated$abrechnungsjahr == year & data_aggregated$bundesland == state, ]$value / by_year[by_year$Group.1 == year & by_year$Group.2 == state, ]$sto_plz
    }
  }
  data_aggregated[is.na(data_aggregated)] <- 0
  data_wide <- spread(data_aggregated, energietraeger, value)
...
bundlands_energietraeger <- relative_freq_interactive(DL_MFH)

p <- plot_ly(type = 'bar')
v = TRUE
for (i in 1:length(states)) {
  p <- p %>% add_trace(y = bundlands_energietraeger[bundlands_energietraeger$bundesland == states[[i]],]$erdgas, name = paste('Erdgas (', states[[i]], ')'),  
                       x = 2002:2018, visible = v)
  p <- p %>% add_trace(y = bundlands_energietraeger[bundlands_energietraeger$bundesland == states[[i]],]$fluessiggas, name = paste('Fluessiggas (', states[[i]], ')'),  
                       x = 2002:2018, visible = v)
  p <- p %>% add_trace(y = bundlands_energietraeger[bundlands_energietraeger$bundesland == states[[i]],]$heizoel, name = paste('Heizoel (', states[[i]], ')'),  
                       x = 2002:2018, visible = v)
  p <- p %>% add_trace(y = bundlands_energietraeger[bundlands_energietraeger$bundesland == states[[i]],]$holzpellets, name = paste('Holzpellets (', states[[i]], ')'),  
                       x = 2002:2018, visible = v)
  p <- p %>% add_trace(y = bundlands_energietraeger[bundlands_energietraeger$bundesland == states[[i]],]$strom, name = paste('Strom (', states[[i]], ')'),  
                       x = 2002:2018, visible = v)
  p <- p %>% add_trace(y = bundlands_energietraeger[bundlands_energietraeger$bundesland == states[[i]],]$waerme, name = paste('Waerme (', states[[i]], ')'),  
                       x = 2002:2018, visible = v)
...
for (state in states) {
  ...
  dropdown_items[[i]] <- list(method = "restyle",
