...
p <- plot_ly(bundSV_mfh_df , x = ~abrechnungsjahr )%>%
  ...
  add_lines(y = ~fitted(lm1),
            line = list(color = '#07A4B5'), showlegend = TRUE) %>%
...
p <- p %>% layout(showlegend=FALSE,
  ...
  updatemenus = list(
    list(
      buttons = get_visibility_combinations_reg_line()
    )
  )
)
...
library(dplyr)
library(ggplot2)
DL_map <- st_read("shapefiles/gadm36_DEU_shp/gadm36_DEU_1.shp",stringsAsFactors = FALSE)
...
## 5g Energy consumption per year MFH (absolute and relative) split by energy source
...
## 5h. Energy consumption per year SFH (absolute and relative) split by energy source
...
tmp <- aggregate(DL_MFH[,c('bundesland','abrechnungsjahr','verbrauch_gesamt_kwh_spez')][c(-1, -2)], by=list(DL_MFH$bundesland, DL_MFH$abrechnungsjahr), FUN = sum)
p <- plot_ly(
...
tmp <- aggregate(bind[c(-1, -2)], 
                 by=list(bind$bundesland, bind$abrechnungsjahr), FUN = sum)
p <- plot_ly(
    x = tmp$Group.2, y = tmp$Group.1,
    z = tmp$verbrauch_gesamt_kwh_spez, type = "heatmap") %>% layout(
