...
When we use interaction between the year and the building type it explains the data better. We next use a linear regression of the specific consumption as a function of temperature as a continuous variable and abrechnungsjahr as a categorical variable (after binning it suitably).
...
In this section we find the total energy produced by the different fuel sources used for heating of residential buildings. Since the data from co2online provide us with the specific energy consumption, we can multiply these with the offical numbers for the areas of residential buildings in Germany. The following figure is an interactive graph showing the proportions of energy produced by the different fuels for multifamily, 1-2 family, and both combined building types. The building type can be chosen from the drop down on the left.

```{r,fig.cap="\\label{fig:figs}Fig. 8: Proportions of energy produced by the different fuels. Please use the drop down menu to choose the building type."}
...
The following figure is an interactive graph showing the co2 emitted in Germany by the various fuel sources. The drop down menu on the left enables one to select the building type.

```{r,fig.cap="\\label{fig:figs}Fig. 9: CO2 emissions by the different fuel types. Please use the drop down menu to choose the building type."}
